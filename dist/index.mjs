...
const parse = (value, options) => {
	let newValue = value;
	if (!isEmpty(value)) if (!isEmpty(options)) newValue = parseHTML(value, options);
	else newValue = parseHTML(value);
	return newValue;
};
...
const isLocalDevelopment = () => {
	...
	try {
		if (typeof import.meta !== "undefined") developmentEnvironment = import.meta.env?.MODE;
	} catch (_error) {}
...
const showStaging = (environmentMode, demonstrationMode) => {
	if ((import.meta.env?.VITE_FORCE_STAGING_MODE === "True" || environmentMode === "staging") && demonstrationMode !== true && environmentMode !== "production" && environmentMode !== "development") return true;
	else return false;
};
