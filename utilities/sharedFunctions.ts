...
export const hasEqualsProperty = (objectItem, propertyName, value): boolean => {
  ...
  if (typeof objectItem === "object") {
    if (
      Object.hasOwn(objectItem, propertyName) &&
      !isEmpty(objectItem[propertyName]) &&
      objectItem[propertyName] === value
    ) {
      equalsProperty = true;
    }
  }
...
export const validateMilitaryTime = (timeEntered): boolean => {
  ...
  if (typeof formatToString(timeEntered) === "string") {
    ...
    if (timeEnteredString.length !== 4) {
      ...
      if (
        isNaN(parseInt(timeEnteredString.charAt(0))) ||
        isNaN(parseInt(timeEnteredString.charAt(1))) ||
        isNaN(parseInt(timeEnteredString.charAt(2))) ||
        isNaN(parseInt(timeEnteredString.charAt(3)))
      ) {
        validTimeFormat = false;
      }

      if (
        timeEnteredString.charAt(0) !== "0" &&
        timeEnteredString.charAt(0) !== "1" &&
        timeEnteredString.charAt(0) !== "2"
      ) {
        validTimeFormat = false;
      }
      ...
      if (
        timeEnteredString.charAt(0) === "2" &&
        timeEnteredString.charAt(1) !== "0" &&
        timeEnteredString.charAt(1) !== "1" &&
        timeEnteredString.charAt(1) !== "2" &&
        timeEnteredString.charAt(1) !== "3" &&
        timeEnteredString.charAt(1) !== "4"
      ) {
        validTimeFormat = false;
      }
      ...
      if (
        timeEnteredString.charAt(0) === "2" &&
        timeEnteredString.charAt(1) !== "4" &&
        timeEnteredString.charAt(2) !== "0" &&
        timeEnteredString.charAt(3) !== "0"
      ) {
        validTimeFormat = false;
      }
      ...
      if (
        timeEnteredString.charAt(2) !== "0" &&
        timeEnteredString.charAt(2) !== "1" &&
        timeEnteredString.charAt(2) !== "2" &&
        timeEnteredString.charAt(2) !== "3" &&
        timeEnteredString.charAt(2) !== "4" &&
        timeEnteredString.charAt(2) !== "5"
      ) {
        validTimeFormat = false;
      }
    }
...
export const removeArticlesFromBeginning = value => {
  ...
  if (!isEmpty(value)) {
    newValue = formatLowerCase(newValue)
      .replace(/^(a\.)/, "")
      .replace(/^(an\.)/, "")
      .replace(/^(the\.)/, "");
  }
...
export const sortObjectArrayByTwoProperties = (
  objectArray,
  sortPropertyOne,
  sortPropertyTwo,
  directionOne,
...
export const getNumberOfDaysBetweenDates = (startDate, endDate): number => {
  ...
  // * The equation (1000 * 60 * 60 * 24) turns the values back into a day count. -- 08/31/2023 KH
  // * The operation + 1 adds one day to account for 08/31/2023 - 08/31/2023 = 0 but should equal 1 day in order to account for running studentsPerDay amount of students that day. -- 08/31/2023 KH
...
export const filterDropdownOptions = (
  ...
  if (!isEmptyArray(allItems)) {
    ...
    filteredItems = allItems.filter(
      item => !itemsToFilter.find(itemToFilter => item[propertyName] === itemToFilter[propertyName])
    );
