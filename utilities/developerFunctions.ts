...
export const showStaging = (environmentMode, demonstrationMode) => {
  ...
  if (
    (forceStagingMode === "True" || environmentMode === "staging") &&
    demonstrationMode !== true &&
    environmentMode !== "production" &&
    environmentMode !== "development"
  ) {
...
export const showPlayground = (environmentMode, demonstrationMode) => {
  ...
  if (
    ...
    ((window.location.href.includes("intranet.orbiseducation.com/test_local/") ||
      window.location.href.includes("intranet.orbiseducation.com/for-review/")) &&
      environmentMode !== "production")
  ) {
    return true;
  } else {
...
export const allowLogging = () => {
  ...
  if (
    ...
    (!isLocalDevelopment() || allowDevelopmentComputerLog === "True")
  ) {
    return true;
  } else {
    return false;
...
export const showAuthentication = (environmentMode, applicationName, azureAuthentication) => {
  ...
  const learningObjectAzureAuthentication =
    azureAuthentication !== true && applicationName === "Learning Object Template";
  ...
  if (
    learningObjectAzureAuthentication === true &&
    window.location.href.includes("orbiseducation.com/test_local/") &&
    environmentMode === "production"
  ) {
    return false;
  } else if (
    !window.location.href.includes("lor.orbiseducation.com") &&
    !window.location.href.includes("lor-dev.") &&
    !window.location.href.includes("lor-staging.") &&
    (window.location.href.includes("orbiseducation.com") ||
      window.location.href.includes("dewfapvs2001e.gce.com"))
  ) {
    return true;
...
export const getFetchAuthorization = (
  ...
  if (
    !isEmpty(databaseNameDevelopment) &&
    (showLocalDevelopment(environmentMode) ||
      showPlayground(environmentMode, demonstrationMode) ||
      window.location.href.includes("lor-dev.orbiseducation.com") ||
      window.location.href.includes("lor-staging.orbiseducation.com"))
  ) {
    databaseName = databaseNameDevelopment;
  }

  return window.btoa(
    JSON.stringify({ partnerID: partnerID, databaseName: databaseName, sessionToken: sessionToken })
  );
};
...
export const resolveBaseURL = (endPointBase, environmentMode, demonstrationMode, lorServer) => {
  ...
  if (
    window.location.hostname.startsWith("lor-dev") ||
    window.location.hostname.startsWith("lor-staging")
  ) {
