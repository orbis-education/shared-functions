    ...
    fetch(url, {
      ...
      .then(results => {
        ...
        if (response.status === 200) {
          ...
          addErrorLog(baseURL, fetchAuthorization, databaseAvailable, allowLogging, { operation: `${operation} SQL Server`, transactionData: { url, response: { ok: response.ok, redirected: response.redirected, status: response.status, statusText: response.statusText, type: response.type, url: response.url }, data, logObject: logObject }, errorData: { message: `${response.status} ${response.statusText} ${response.url}` }, dateEntered: getDateTime() });
      ...
      .catch((error) => {
        ...
        addErrorLog(baseURL, fetchAuthorization, databaseAvailable, allowLogging, { operation, transactionData: { url, response: { ok: response.ok, redirected: response.redirected, status: response.status, statusText: response.statusText, type: response.type, url: response.url }, data, logObject }, errorData: { name: error.name, message: error.message, stack: error.stack }, dateEntered: getDateTime() });
